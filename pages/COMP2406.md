...
* When you ship JavaScript you are shipping source code
* The thing you are selling is the access the software gives

* People are 'disguising the code' so that they can hopefully sell it without just giving out their source code outright

### Definition changes

#### Responsive

* Word has changed meaning
* I now means that your web page will respond to different resolutions, devices, formats etc.

#### Serverless

* You will rent the server off of amazon, or some other hosting space

### Simple web apps

* Client gets served everything and then it just updates, doesn't need to get more from the server

### Single Page App with JSON data

* client gets served html,.js and then when the client clicks the client and server exchange JSON files
* The server is writing and reading from the disk - **NOT** a database though
* All ports 3000 - 3010 are what we will be using
* Don't use ports <1000 (usually important ports already being used)

**Keep an eye on the web technologies page on the website**

### Tutorial 2 - Client server JSON Data

#### What is this?

* html file
    * which has a canvas in it
    * and some buttons
    * favicon
    * it also has a script tag for the JavaScript
    ```
    <script src='canvasWithTimer.js'></script>
    ```
    * html are dead with webapps, only used for delivery of .js
    * You usually don't want to mention js function in and html document
    * You can used html id's so that the js can target it
    * id's shouldn't apear more than once, for that use classes

* jquery js file
* canvasWithTimer js file

* When something comes from the server does it come as  a JSON string or a Js Object ?
* Just be prepared to inspect what is happening

### A warning about semi-colons

* javascript will insert semicolons where it thinks there should be one

### $

The $ is just a letter in javascript

jQuery used $ as a varialbe name to access the jquery library
...
### Main debugging strategy

* Will be printing console statements to both the server and the client
